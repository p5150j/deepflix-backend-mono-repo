...
def merge_video_audio(video_path, audio_path, output_path):
    ...
    try:
        ...
        if video_duration > audio_duration:
            ...
            merge_cmd = f"ffmpeg -i {video_path} -i {audio_path} -filter_complex '[1:a]apad[a1]' -map 0:v -map '[a1]' -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 192k -shortest {output_path}"
        else:
            # If audio is longer or equal, just merge normally
            merge_cmd = f"ffmpeg -i {video_path} -i {audio_path} -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 192k -shortest {output_path}"
...
def generate_narration(text, image_path, output_folder, logger):
    ...
    try:
        ...
        # print(f"Request data: {json.dumps(data, indent=2)}")  # Commented out excessive logging
        ...
        # print(f"TTS API info response: {json.dumps(info_data, indent=2)}")  # Commented out excessive logging
        ...
        # print(f"Response Headers: {json.dumps(dict(response.headers), indent=2)}")  # Commented out excessive logging
