...
def build_image_workflow(sequence_data, character_data, seed, sampler, steps, cfg_scale, output_folder, global_negative_prompt=None):
    ...
    for scene in sequence_data:
        ...
        # Anatomy issues with high weights
        anatomy_issues = "(extra digits:1.8), (extra arms:1.8), (extra hands:1.8), (fused fingers:1.8), (malformed limbs:1.8), (mutated hands:1.8), (poorly drawn hands:1.8), (extra fingers:1.8), (missing hands:1.8), (bad hands:1.8), (three hands:1.8), (fused hands:1.8), (too many fingers:1.8), (disproportionate limbs:1.8), (distorted anatomy:1.8), (unnatural poses:1.8)"
        
        # NSFW content with moderate weights
        nsfw_content = "(NSFW:1.5), (nude:1.5), (uncensored:1.5), (cleavage:1.5), (nipples:1.5), (explicit content:1.5), (adult content:1.5), (inappropriate content:1.5)"
        
        # General quality issues
        quality_issues = "(worst quality:1.4), (low quality:1.4), (blurry:1.2), (pixelated:1.2), (watermark:1.2), (signature:1.2), (text:1.2), (logo:1.2), (distorted face:1.8), (warping:1.8), (glitching:1.8), (jittering:1.8)"
        
        if scene_type == "character":
            # Character-specific issues
            character_issues = "(multiple people:1.8), (two women:1.8), (two persons:1.8), (twins:1.8), (multiple subjects:1.8), (group of people:1.8), (duplicated character:1.8), (double character:1.8), (different person:1.8), (inconsistent character:1.8), (different face:1.8), (changing face:1.8), (morphing face:1.8)"
            negative_text = f"{base_negative}, {scene_negative}, {anatomy_issues}, {nsfw_content}, {quality_issues}, {character_issues}"
        else:
            # For non-character scenes, exclude character-specific issues
            negative_text = f"{base_negative}, {scene_negative}, {anatomy_issues}, {nsfw_content}, {quality_issues}"
...
def build_video_workflow(image_path, clip_action, output_folder, clip_duration=3.0625, transition_type="none"):
    ...
    # Video-specific issues - more concise version
    video_issues = "(unnatural movement:1.5), (melting face:1.5), (morphing:1.5), (duplicated features:1.5), (flickering:1.5), (artifacts:1.5), (grid pattern:1.8), (checker pattern:1.8)"
    
    # Combine all negative prompts - keeping it minimal for video
    negative_prompt = f"{video_issues}, (worst quality:1.2), (low quality:1.2), (blurry:1.1), (distorted face:1.5), (warping:1.5), (glitching:1.5)"
...
def merge_video_audio(video_path, audio_path, output_path):
    """
    # TODO: Memory Management and Validation Improvements Needed:
    # 1. Add explicit memory cleanup after ffmpeg operations
    # 2. Add file validation before and after each operation
    # 3. Add sleep/cooldown periods between heavy operations
    # 4. Add detailed logging of memory usage before/after operations
    # 5. Consider implementing a resource monitor to track GPU/memory usage
    # 6. Add graceful cleanup on failure
    """
    try:
...
def concatenate_videos(output_folder):
    ...
    try:
        ...
        # Add background music after successful video creation
        if add_background_music(output_folder):
            return True
        else:
            print("❌ Failed to add background music, but video was created successfully")
            return True
...
def generate_narration(text, image_path, output_folder, logger):
    """Generate narration audio using TTS API"""
    try:
        base_filename = os.path.splitext(os.path.basename(image_path))[0]
        ...
        if not text or text.strip() == "...":
            ...
            video_file = os.path.join(output_folder, f"{base_filename}_00001.mp4")  # Using 1 underscore since base has one
            audio_file = os.path.join(output_folder, f"{base_filename}_00001.wav")  # Using 1 underscore since base has one
...
def process_video_generation(folder_id, sequence_data):
    ...
    for item in sequence_data:
        ...
        video_file = os.path.join(output_folder, f"{base_name}__00001.mp4")  # Using 2 underscores
        
        # Generate video using the correct image pattern and clip_action
        success = generate_video(
            os.path.join(output_folder, f"{base_name}.png"),
            video_file,
            item.get("clip_duration", 3.0625),
            item.get("clip_action")  # Pass the clip_action from sequence data
        )
    ...
    for item in sequence_data:
        ...
        # Update the base name to match the exact pattern we see in the directory
        base_name = f"scene_{format_sequence_number(scene_number)}_{item.get('type', 'character')}_00001_"
        audio_file = os.path.join(output_folder, f"{base_name}__00001.wav")  # Using 2 underscores
    ...
    for video_file in video_files:
        ...
        # Try all possible audio file patterns
        audio_patterns = [
            os.path.join(output_folder, f"{base_name}__00001.wav"),  # Silent audio pattern
            os.path.join(output_folder, f"{base_name}__00001_.wav"),  # TTS audio pattern
            os.path.join(output_folder, f"{base_name}___00001_.wav")  # Old TTS pattern
        ]
...
def wait_for_completion(prompt_id, max_retries=300, retry_interval=2):
    """Wait for ComfyUI to complete the video generation"""
    print("\n⏳ Waiting for video generation to complete...")
    retries = 0
    ...
    while retries < max_retries:
        try:
            ...
            if queue_response.status_code != 200:
                print(f"❌ Failed to check queue status: {queue_response.text}")
                return False
            ...
            if history_response.status_code != 200:
                print(f"❌ Failed to check history: {history_response.text}")
                return False
            ...
            # If prompt is still in queue, wait
            if any(prompt_id in item for item in queue_data.get("queue_running", [])):
                print(f"⏳ Still processing... ({retries}/{max_retries})")
            elif any(prompt_id in item for item in queue_data.get("queue_pending", [])):
                print(f"⏳ Waiting in queue... ({retries}/{max_retries})")
            else:
                print("❌ Prompt not found in queue or history")
                return False
